...
| 16 notifications-marketing-support | ✅ done | one send path for push/SMS/email/in-app with dedupe and consent, bn/en templates with tenant overrides, event hooks across order→delivery→refund, push campaigns + abandoned-cart nudges, per-tenant GA4/Meta with server-side purchase events, support tickets with internal notes; 373 api tests |
